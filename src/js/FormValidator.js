export default class FormValidator {
  ...
  setSubmitButtonState(statement) {                                                    //включение/выключение кнопки submit, принмает true либо false      
    const curButton = this.formElement.querySelector('.button')

    if (!statement) {
      curButton.setAttribute("disabled", "");
      curButton.classList.add('popup__button_state_disable');
    }
    else {
      curButton.removeAttribute("disabled", "");
      curButton.classList.remove('popup__button_state_disable');
    }
  }
